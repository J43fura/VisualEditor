...
ve.dm.Surface.prototype.getHistory = function () {
  var appliedUndoStack = this.undoStack.slice(
    0,
...
ve.dm.Surface.prototype.pushStaging = function (allowUndo) {
  ...
  this.stagingStack.push({
    ...
    selectionBefore: this.isStaging()
      ? this.getStaging().selectionBefore
...
ve.dm.Surface.prototype.popStaging = function () {
  ...
  var transactions = staging.transactions;
  var reverseTransactions = [];

  // Not applying, so rollback transactions
  for (var i = transactions.length - 1; i >= 0; i--) {
    var transaction = transactions[i].reversed();
...
ve.dm.Surface.prototype.canUndo = function () {
  return (
    ...
    !this.readOnly &&
    (!this.isStaging() || this.doesStagingAllowUndo()) &&
    !this.undoConflict
  );
};
...
ve.dm.Surface.prototype.hasBeenModified = function () {
  return (
    this.undoStack.length - this.undoIndex > 0 || !!this.newTransactions.length
  );
};
...
ve.dm.Surface.prototype.getFragment = function (
  ...
  return this.sourceMode
    ? new ve.dm.SourceSurfaceFragment(
        this,
        selection,
        noAutoSelect,
        excludeInsertions
      )
    : new ve.dm.SurfaceFragment(
        this,
        selection,
        noAutoSelect,
        excludeInsertions
 ...
 * @return {ve.dm.SurfaceFragment} Surface fragment
 */
ve.dm.Surface.prototype.getLinearFragment = function (
  range,
  noAutoSelect,
  excludeInsertions
) {
...
ve.dm.Surface.prototype.fixupRangeForLinks = function (range) {
  ...
  var end = range.end;
  var rangeAnnotations = linearData.getAnnotationsFromRange(range);
  var startLink = getLinks(start).diffWith(rangeAnnotations).getHash(0);
  var endLink = getLinks(end).diffWith(rangeAnnotations).getHash(0);
  ...
  if (endLink !== undefined) {
    while (
      end < linearData.getLength() &&
      getLinks(end).containsHash(endLink)
    ) {
      end++;
    }
  }
...
ve.dm.Surface.prototype.setSelection = function (selection) {
  ...
  if (
    ...
    ve.error(
      "Attempted to set an out of bounds selection: " +
        JSON.stringify(selection) +
        ", adjusting"
    );
    // Fix up the selection so things don't break if the caller subsequently
    // tries to use the selection
    selection = new ve.dm.LinearSelection(
      new ve.Range(Math.min(maxOffset, selection.getRange().start), maxOffset)
    );
  ...
  var selectedNode;
  var branchNodes = {};
  var contextChange = false;
  if (selection instanceof ve.dm.LinearSelection) {
    range = selection.getRange();

    // Update branch nodes
    branchNodes.start = this.getDocument().getBranchNodeFromOffset(range.start);
    branchNodes.startRange = intersectRanges(
      branchNodes.start.getRange(),
      range
    );
    if (!range.isCollapsed()) {
    ...
    if (!this.sourceMode) {
      ...
      if (
        selectionChange &&
        !range.isCollapsed() &&
        oldSelection instanceof ve.dm.LinearSelection
      ) {
        var rangeFocus = new ve.Range(range.to);
        var oldRangeFocus = new ve.Range(oldSelection.getRange().to);
        var focusRangeMovingBack = rangeFocus.to < oldRangeFocus.to;
        ...
        if (
          !linearData
            .getInsertionAnnotationsFromRange(rangeFocus, focusRangeMovingBack)
            .compareTo(
              linearData.getInsertionAnnotationsFromRange(
                oldRangeFocus,
                focusRangeMovingBack
              )
            )
        ) {
          contextChange = true;
  ...
  if (
    ...
    branchNodes.startRange &&
    this.branchNodes.startRange &&
    (branchNodes.startRange.isCollapsed() !==
      this.branchNodes.startRange.isCollapsed() ||
      branchNodes.endRange.isCollapsed() !==
        this.branchNodes.endRange.isCollapsed())
  ) {
 ...
 * @fires contextChange
 */
ve.dm.Surface.prototype.changeInternal = function (
  transactions,
  selection,
  skipUndoStack
) {
  ...
  var selectionAfter = this.selection;
...
ve.dm.Surface.prototype.breakpoint = function () {
  ...
  if (this.newTransactions.length > 0) {
    this.undoStack.push({
      start:
        this.getDocument().getCompleteHistoryLength() -
...
ve.dm.Surface.prototype.undo = function () {
  ...
  if (!this.isMultiUser()) {
    ...
    if (item) {
      ...
      for (var i = item.transactions.length - 1; i >= 0; i--) {
    ...
    while (this.undoIndex <= this.undoStack.length) {
      ...
      var authorId = item.transactions[0].authorId;
    ...
    if (item) {
      ...
      var result = ve.dm.Change.static.rebaseUncommittedChange(
        history,
        done.reversed()
      );
      if (result.rejected) {
        ...
        var selection = item.selectionBefore.translateByChange(
...
ve.dm.Surface.prototype.redo = function () {
  ...
  var item = this.undoStack[this.undoStack.length - this.undoIndex];
  if (item) {
    this.undoIndex--;
...
ve.dm.Surface.prototype.getSelectedNodeFromSelection = function (selection) {
  ...
  if (!range.isCollapsed()) {
    var startNode = this.getDocument().documentNode.getNodeFromOffset(
      range.start + 1
    );
...
ve.dm.Surface.prototype.storeChanges = function () {
  ...
  if (!change.isEmpty()) {
    var changes =
      this.storage.getObject(this.autosavePrefix + "ve-changes") || [];
    changes.push(change);
    if (
      ...
      this.storage.setObject(
        this.autosavePrefix + "ve-selection",
        this.getSelection(),
        this.storageExpiry
      );
...
ve.dm.Surface.prototype.storeDocStorage = function () {
  ...
  var dmDoc = this.getDocument();
  this.storage.setObject(
    this.autosavePrefix + "ve-docstorage",
    dmDoc.getStorage(),
    this.storageExpiry
  );
...
ve.dm.Surface.prototype.setStorage = function (storage, storageExpiry) {
  if (this.storing) {
    throw new Error(
      "Can't change storage interface after auto-save has stared"
    );
  }
...
ve.dm.Surface.prototype.storeDocState = function (state, html) {
  ...
  if (
    !this.storage.set(
      this.autosavePrefix + "ve-dochtml",
      useLatestHtml ? this.getHtml() : html,
      this.storageExpiry
    )
  ) {
...
ve.dm.Surface.prototype.updateDocState = function (state) {
  if (
    this.storage.set(
      this.autosavePrefix + "ve-docstate",
...
ve.dm.Surface.prototype.updateExpiry = function (skipKeys) {
  ...
  var surface = this;
  skipKeys = skipKeys || [];
  ["ve-docstate", "ve-dochtml", "ve-selection", "ve-changes"].forEach(function (
    key
  ) {
    if (skipKeys.indexOf(key) === -1) {
      surface.storage.setExpires(
        surface.autosavePrefix + key,
        surface.storageExpiry
      );
    }
  });
};
