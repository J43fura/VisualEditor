...
ve.ui.DiffElement = function VeUiDiffElement(visualDiff, config) {
  ...
  if (visualDiff.timedOut) {
    var $warning = this.constructor.static.createWarning(
      ve.msg("visualeditor-diff-timed-out")
    );
  ...
  console.log(this.descriptions.$element);
};
 ...
 */
ve.ui.DiffElement.static.compareAttributes = function (
  oldAttributes,
  newAttributes
) {
...
ve.ui.DiffElement.prototype.onDescriptionsHighlight = function (item) {
  ...
  if (item) {
    ...
    var elementRects = ve.ce.FocusableNode.static.getRectsForElement(
      this.getDiffElementById(item.getData()),
      overlayRect
    ).rects;
...
ve.ui.DiffElement.prototype.positionDescriptions = function () {
  ...
  this.descriptions.getItems().forEach(function (item) {
    ...
    var elementRect =
      ve.ce.FocusableNode.static.getRectsForElement($element).boundingRect;
 ...
 */
ve.ui.DiffElement.prototype.renderQueue = function (
  queue,
  parentNode,
  spacerNode
) {
  ...
  queue.forEach(function (item) {
    if (item) {
      ...
      while (elements.length) {
        parentNode.appendChild(parentNode.ownerDocument.adoptNode(elements[0]));
        elements.shift();
      }
    } else {
...
ve.ui.DiffElement.prototype.renderDiff = function (
  ...
  function handleRefList(node, move) {
    if (
      ...
      (referencesListDiff =
        referencesListDiffs[node.element.attributes.listGroup])
    ) {
      ...
      // just get that.
      diffQueue.push([
        "getRefListNodeElements",
        referencesListDiff.element,
        referencesListDiff.action,
        move,
        referencesListDiff.descriptionItemsStack,
      ]);
  ...
  if (!documentNode.children.length) {
    ...
    noChanges.appendChild(
      document.createTextNode(ve.msg("visualeditor-diff-no-changes"))
    );
...
ve.ui.DiffElement.prototype.renderMetaListDiff = function (
  metaListDiff,
  documentNode,
  documentSpacerNode
) {
...
ve.ui.DiffElement.prototype.wrapNodeData = function (nodeDoc, nodeData) {
  ...
  var nodeElements = ve.dm.converter.getDomFromModel(
    documentSlice,
    ve.dm.Converter.static.PREVIEW_MODE
  ).body;
 ...
 * @return {HTMLElement[]} Elements (not owned by window.document)
 */
ve.ui.DiffElement.prototype.getChangedNodeElements = function (
  diff,
  oldNode,
  newNode,
  move
 ...
 */
ve.ui.DiffElement.prototype.getChangedNodeData = function (
  diff,
  oldNode,
  newNode,
  move,
...
ve.ui.DiffElement.prototype.getChangedLeafNodeData = function (newNode, diff) {
  ...
  if (attributeChange) {
    ...
    this.addAttributesToElement(nodeData, 0, {
 ...
 * @return {number}
 */
ve.ui.DiffElement.prototype.appendListItem = function (
  diffData,
  insertIndex,
  listNode,
  listNodeData,
 ...
 * @return {Array} Linear data for the diff
 */
ve.ui.DiffElement.prototype.getChangedDocListData = function (
  newDoclistNode,
  diff,
  neverProcess
) {
  ...
  this.addAttributesToElement(spacerData, 0, {
  ...
  var newDoclistNodeData =
    this.constructor.static.getDataFromNode(newDoclistNode);
  if (diff.attributeChange) {
    var item = this.compareNodeAttributes(
      newDoclistNodeData,
      0,
  ...
  diffQueue.forEach(function (diffItem) {
    if (diffItem) {
      ve.batchPush(
        diffData,
        diffElement[diffItem[0]].apply(diffElement, diffItem.slice(1))
      );
    } else {
 ...
 * @return {Array} Linear data for the diff
 */
ve.ui.DiffElement.prototype.getChangedListNodeData = function (
  newListNode,
  diff
) {
  ...
  processedListDiffItems.forEach(function (item) {
    ...
    if (!item.diff) {
      if (!item.node) {
        ...
        isSpacer = true;
        diffElement.addAttributesToElement(contentData, 0, {
          "data-diff-action": "none",
        });
      } else {
        // Get the linear data for the list item's content
        contentData = diffElement.getNodeData(
          item.node,
          item.action,
          item.move || null
        );
    ...
    if (listNode !== lastListNode) {
      listNodeData = [
        diffElement.constructor.static.getDataFromNode(listNode)[0],
      ];
    ...
    var listItemData =
      diffElement.constructor.static.getDataFromNode(listItemNode);
    if (isSpacer) {
    ...
    if (item.diff && item.diff.attributeChange) {
      [
        "depthChange",
        "listNodeAttributeChange",
        "listItemAttributeChange",
      ].forEach(function (listChangeType) {
 ...
 * @return {Array} Linear data for the diff
 */
ve.ui.DiffElement.prototype.getChangedTreeNodeData = function (
  oldTreeNode,
  newTreeNode,
  diff
) {
  ...
  function highlightRemovedNode(nodeIndex) {
    ...
    if (node.isDiffedAsTree() && node.hasChildren()) {
      ...
      var removeData = getRemoveData.call(
        this,
        orderedNode,
        highestRemovedAncestor.index
      );
      var insertIndex;
      ...
      if (highestRemovedAncestor.index in highestRemovedAncestors) {
        ...
        insertIndex =
          newNodes[
            correspondingNodes.oldToNew[highestRemovedAncestor.index]
        ...
        for (var x = 0, xlen = siblingNodes.length; x < xlen; x++) {
          if (siblingNodes[x].index === highestRemovedAncestor.index) {
            ...
            if (
              correspondingNodes.oldToNew[oldPreviousNodeIndex] !== undefined
            ) {
              newPreviousNodeIndex =
                correspondingNodes.oldToNew[oldPreviousNodeIndex];
            }
          }
        }
        ...
        if (newPreviousNodeIndex !== undefined) {
          insertIndex =
            newNodes[newPreviousNodeIndex].node.getOuterRange().to -
            nodeRange.from;
        } else {
          newPreviousNodeIndex =
            correspondingNodes.oldToNew[highestRemovedAncestor.parent.index];
          insertIndex =
            newNodes[newPreviousNodeIndex].node.getRange().from -
            nodeRange.from;
        }
  ...
  function highlightInsertedNode(nodeIndex) {
    ...
    // Add insert class
    this.addAttributesToElement(nodeData, nodeRangeStart, {
      "data-diff-action":
        node.isDiffedAsTree() && node.hasChildren()
          ? "structural-insert"
          : "insert",
    });
  }
  ...
  function highlightChangedNode(oldIdx, newIdx, info) {
    ...
    var nodeDiffData = diffElement.getChangedNodeData(
      info,
      oldNode,
      newNode,
      null,
 ...
 */
ve.ui.DiffElement.prototype.getRefListNodeElements = function (
  referencesListContainer,
  action,
  move,
  items
 ...
 */
ve.ui.DiffElement.prototype.getInternalListChangedNodeElements = function (
  diff,
  oldNode,
  newNode,
  move,
  ...
  var documentSlice = this.newDoc.cloneWithData(data, true, true);
  var body = ve.dm.converter.getDomFromModel(
    documentSlice,
    ve.dm.Converter.static.PREVIEW_MODE
  ).body;
 ...
 */
ve.ui.DiffElement.prototype.compareNodeAttributes = function (
  data,
  offset,
  attributeChange
) {
  var attributeChanges = this.constructor.static.compareAttributes(
    attributeChange.oldAttributes,
    attributeChange.newAttributes
  );

  var changes = ve.dm.modelRegistry
    .lookup(data[offset].type)
  ...
  if (
    ...
    this.addAttributesToElement(data, offset, {
 ...
 */
ve.ui.DiffElement.prototype.getChangeDescriptionItem = function (
  changes,
  classes
) {
 ...
 */
ve.ui.DiffElement.prototype.addAttributesToElement = function (
  data,
  offset,
  attributes
) {
...
ve.ui.DiffElement.prototype.markMove = function (move, elementOrData, offset) {
  ...
  var item = this.getChangeDescriptionItem(
    [ve.msg("visualeditor-diff-moved-" + move)],
    ["ve-ui-diffElement-moved-" + move]
  );
  if (Array.isArray(elementOrData)) {
    this.addAttributesToElement(elementOrData, offset || 0, {
      "data-diff-move": move,
      "data-diff-id": item.getData(),
    });
  } else {
...
ve.ui.DiffElement.prototype.annotateNode = function (linearDiff) {
  ...
  for (i = 0; i < ilen; i++) {
    ...
    if (start !== end) {
      ...
      if (type !== 0) {
        ...
        var iid = 0;
        domElement.setAttribute("ID", iid++);
        ...
        if (linearDiff[i].annotationChanges) {
          ...
          linearDiff[i].annotationChanges.forEach(function (annotationChange) {
            ...
            if (
              annotationChange.oldAnnotation &&
              annotationChange.newAnnotation
            ) {
              attributeChanges =
              ...
              changes = changes.concat(
                ve.dm.modelRegistry
                  .lookup(annotationChange.newAnnotation.getType())
                  .static.describeChanges(
                    attributeChanges,
                    annotationChange.newAnnotation.getAttributes(),
                    annotationChange.newAnnotation.getElement()
                  )
              );
            } else if (annotationChange.newAnnotation) {
              changes = changes.concat(
                annotationChange.newAnnotation.describeAdded()
              );
            } else if (annotationChange.oldAnnotation) {
              changes = changes.concat(
                annotationChange.oldAnnotation.describeRemoved()
              );
            }
          });
        }
        ...
        var originalDomElementsHash = diffDoc
          .getStore()
          .hash(domElements, domElements.map(ve.getNodeHtml).join(""));
        ...
        var height = Math.min(
          ve.getCommonStartSequenceLength(
            annHashLists.slice(Math.max(0, range.start - 1), range.end)
          ),
          ve.getCommonStartSequenceLength(
            annHashLists.slice(
              range.start,
        ...
        for (j = range.start; j < range.end; j++) {
          ...
          diffDoc.data.setAnnotationHashesAtOffset(j, annHashLists[j]);
        }
      }
    }
    start = end;
  ...
  var annotatedLinearDiff = diffDoc.getData({
    start: 0,
